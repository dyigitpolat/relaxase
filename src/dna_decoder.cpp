...
int DNADecoder::DNAtoOrd(std::string strand){
  int res=0;
  for(int i=0; i<8; i++){
     res=res<<2;
     switch(strand.at(i))
...
unsigned char DNADecoder::DNAtoC1(std::string strand, int row){
  unsigned char res=0;
  for(int i=0; i<4; i++){
     res= res<<2;
...
unsigned char DNADecoder::DNAtoC2(std::string strand, int row){
  unsigned char res=0;
  for(int i=0; i<4; i++){
     res= res<<2;
...
uint8_t DNADecoder::placeBitAccToPriority(int positionInChar, /*comes from the priority array(where the bit needs to be placed)*/
...
void DNADecoder::WriteRSMatrixByPriority(int inputSize, char *priority_file, char decoded_matrix[][2*data_length],char *output_file)
{
...
void DNADecoder::WriteRSMatrixByColumn(int inputSize, char decoded_matrix[][data_length*2],char* output_file){
...
int DNADecoder :: decode_file(string file_name, int file_size, int mapping_format, int skip_ECC_enable, char * proirity_def_file, char * output_file)
{

    int inputSize=file_size;
    int total_symbols = RS_ROWS*dcode_length;
    int total_bytes   = 2*total_symbols;

    unsigned char input_bytes[RS_ROWS][2*dcode_length];
    ...
    int erasures[dcode_length]={0};
    int erasure_count=dcode_length;
    ...
    for(int i=0; i< dcode_length; i++)
    {
        if(erasures[i] == 0)
            erasure_location_list.push_back(i);
    }

    uint16_t RS_matrix[RS_ROWS][dcode_length];
    std::memset(&RS_matrix[0][0], 0, total_bytes);

    for(int i=0; i<RS_ROWS; i++)
      for(int j=0; j<dcode_length; j++)
   ...
   // const schifra::galois::field field(field_descriptor,
    ...
    //typedef schifra::reed_solomon::decoder<dcode_length,fec_length,data_length> decoder_t;
 ...
 //#pragma omp parallel for
      ...
      //{
        //  detected[i]=corrected[i]=0;
         ...
         //schifra::reed_solomon::block<dcode_length,fec_length> block;
         //schifra::reed_solomon::copy(&RS_matrix[i][0], dcode_length, block);
    ...
    if(mapping==0) WriteRSMatrixByColumn(inputSize, decoded_matrix, output_file);
    else WriteRSMatrixByPriority(inputSize, proirity_def_file, decoded_matrix, output_file);
